...
| `lander ride --date <when>` / `--time <minutes>` / `--await <ids>` | Have **this** task ride again when a wakeup trigger fires (a time, or other tasks landing); once its turn ends it rests until then, and resumes with a generated message naming the trigger that fired — "Resumed at …" for a time, "Tasks landed: …" for an await. |
...
`lander ride` takes the same `--date`/`--time`/`--await` flags (at least one required) to re-sleep a running task: it records a scheduled or awaiting event, rests once its turn ends, and, when the scheduler wakes it, resumes with a generated message naming the trigger that fired: `Resumed at <time>` for a time, `Tasks landed: <ids>` for an await. Only a time trigger carries a timestamp — for the others the moment is not the condition that was met. A task launched by hand from the UI, ahead of any trigger, takes the time wording too; nothing may claim a condition that never fired. `lander ride --clear` disarms pending wakeup triggers without touching status or rewriting history. The CLI is a zero-dependency Node script that talks to the local HTTP API, so the server stays the single source of truth; actual agent subprocesses are launched by the daemon, not by the CLI.
