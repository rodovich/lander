...
| `lander rest --date <when>` / `--time <minutes>` / `--await <ids>` | Put **this** task to rest until a wakeup trigger fires (a time, or other tasks landing); it resumes then with a generated message naming the trigger that fired — "Resumed at …" for a time, "Tasks landed: …" for an await. |
...
`lander rest` takes the same `--date`/`--time`/`--await` flags (at least one required) to re-sleep a running task: it flips to resting with a scheduled or awaiting event and, when the scheduler wakes it, resumes with a generated message naming the trigger that fired: `Resumed at <time>` for a time, `Tasks landed: <ids>` for an await. Only a time trigger carries a timestamp — for the others the moment is not the condition that was met. A task launched by hand from the UI, ahead of any trigger, takes the time wording too; nothing may claim a condition that never fired. `lander rest --clear` disarms pending wakeup triggers without touching status or rewriting history. The CLI is a zero-dependency Node script that talks to the local HTTP API, so the server stays the single source of truth; actual agent subprocesses are launched by the daemon, not by the CLI.
