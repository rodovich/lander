...
| `lander ask --option id:label...` | Raise an **advisory** choice question on **this** task *without* wedging — the task keeps pacing and nothing appears in the list. The option buttons render under your message and the chosen option's label arrives as the next user message. The user may instead just reply, which supersedes the ask. (Choice-only: a free-text ask would just duplicate the composer.) |
| `lander ride --date <when>` / `--time <minutes>` / `--await <ids>` | Have **this** task ride again when a wakeup trigger fires (a time, or other tasks landing); once its turn ends it waits until then, and resumes with a generated message naming the trigger that fired — "Resumed at …" for a time, "Tasks landed: …" for an await. |
| `lander ride --clear` | Drop **this** task's pending wakeup triggers (the time/await a prior `ride` armed) — for when it was woken early and the scheduled resume is no longer wanted. |
...
| `lander attachment ls` / `lander attachment cat <id>` | List **this** task's files (id/name/mime/size) — both the ones sent to it and the ones it attached — or stream one's raw bytes to stdout. See [Attachments](#attachments). |
| `lander attachment put <path> [--name <n>] [--mime <m>]` | Attach a file to the turn **this** task is riding, so it renders on the reply (name defaults to the basename) — see [Outputs](#outputs). |
...
`land`, `wedge`, and `ride` act on the current task via `LANDER_TASK`; `launch`, `list`, `view`, `send`, and `archive` only need `LANDER_API`/`LANDER_PROJECT`. `list`, `view`, `send`, and `archive` are scoped to the caller's own project: `list` accepts `--status <s>` to filter, `--since`/`--until <when>` to bound the range by createdAt, `--text <terms>` to search title and message text (a term equal to a task's exact id also matches), and `--json` for structured metadata. `--status <s>` takes one of `landed`, `riding`, `pacing`, `wedged`, and anything else is rejected rather than filtered with. `--status landed` and a `--since`/`--until` window read the archive alongside the active pool, since that is where most finished work ends up, and mark the rows they found there. A query with `--text` or a live status (`riding`, `pacing`, `wedged`), or with no filter at all, reads the active pool alone; `--archived` reads the archive alone. In all cases `list` returns metadata only, never task conversations. `view` accepts `--json` for the full task. `send` reads its message from the argument or stdin (`-`), and leads the delivered message with a bare `From <sender id>:` backlink to the sending task. Every command that takes an `<id>` wants the whole id, as `list`, `launch`, and `view` print it.

With no trigger flag, `send` delivers immediately, queued behind any in-flight turn. With `--date`/`--time`/`--await` (same semantics as `launch`/`ride`) it stashes the message on the recipient as a scheduled message that fires when its time arrives or every awaited task has landed, whichever comes first. A task may only message tasks in its own project.

`lander launch` reads the message from the argument, or from stdin if it's `-`, and accepts `--project <slug>`, `--title <title>`, `--date <when>` / `--time <minutes>` / `--await <ids>`, and `--edits`. `--title` names the task directly instead of using the auto-titler. Deferred launches are created pacing and the scheduler wakes them when their trigger fires; a pacing scheduled task also gets a **Launch** item in its row's kebab menu (see [Frontend](#frontend-src)) to run it early. A spawned task is read-only unless `--edits` is passed, and `--edits` is inherit-only: a child may be granted edit access only if the spawning task already has it. (Git isn't a Lander grant — it's governed by the project's `.claude` settings.) A human can later grant edits to a read-only task from the crossed-out-pencil menu in the task header. As a guard against a mistyped flag becoming work, `launch` (and `send`/`relaunch`) refuse a message that is a lone flag-shaped token like `--help` or `--titel`; pass such a string as content via `--message` or `-` (stdin).

`lander ride` takes the same `--date`/`--time`/`--await` flags (at least one required) to re-sleep a running task: it records a scheduled or awaiting event and, once its turn ends and the scheduler wakes it, resumes with a generated message naming the trigger that fired: `Resumed at <time>` for a time, `Tasks landed: <ids>` for an await. Only a time trigger carries a timestamp — for the others the moment is not the condition that was met. A task launched by hand from the UI, ahead of any trigger, takes the time wording too; nothing may claim a condition that never fired. `lander ride --clear` disarms pending wakeup triggers without touching status or rewriting history. The CLI is a zero-dependency Node script that talks to the local HTTP API, so the server stays the single source of truth; actual agent subprocesses are launched by the daemon, not by the CLI.

#### Attachments

Messages can carry file/image attachments, propagated to both Claude and Codex agents. From the web UI, a **paperclip** below the new-task and reply composers attaches files; from the CLI, `lander launch` and `lander send` take `--attach <paths…>` (variadic — put it last, after the message, since it consumes following args up to the next `--flag`). Attachments are uploaded to a durable per-project blob store (`data/<project>/attachments/`, an `<id>` blob plus an `<id>.json` metadata sidecar) and carried on the message as refs (`{id, name, mime, size}`) — never inlined into the prompt.

At turn time the daemon lazily materializes a task's attachments into a per-task dir on its host (`LANDER_FILES_DIR`, cached across turns), writes a `manifest.json`, and appends a small **manifest block** (ids/names/sizes, never the bytes) to the outgoing prompt. Images additionally reach the model's **vision**: Codex via `--image`, Claude via `Read` on the local path (the daemon grants `--add-dir` for the store dir so Read can reach it). To read any attached file's bytes an agent runs `lander attachment cat <id>` (a pure local read of `$LANDER_FILES_DIR/<id>` — no server call, no sandbox widening), and `lander attachment ls` lists the current task's files with sizes, each prefixed with the turn it belongs to (`1/report.md`) so two files sharing a name stay distinguishable.

#### Outputs

A task's **outputs** are the same thing as its inputs, differing only in which message carries them: `lander attachment put <path> [--name <n>] [--mime <m>]` stores a blob and records the ref on the flow item for the turn the task is riding, so the file renders as a chip on that reply. If the turn has emitted nothing yet, the attachment opens its first item and becomes the first thing that turn shows — a flow that writes its outputs before it speaks still has them appear where it wrote them.

An HTML output previews as a live page — a component harness with dummy data, an interactive chart — rather than as source. It runs in an iframe sandboxed to an opaque origin (scripts allowed, no access to the lander page) under a CSP that permits only inline and `data:`/`blob:` resources, so the page must be one self-contained file: bundle scripts, styles, and assets inline (e.g. esbuild or `vite-plugin-singlefile`). The preview's **Open in tab** shows it full-window under the same sandbox.

Only the task itself (or the human) may attach to a task's turn: uploading a blob is a project-wide right, but a ref on task T's assistant turn is a claim about what T produced. There is no separate read route — the ref carries a blob id and the project-scoped download serves it — and `put` also caches the blob into `LANDER_FILES_DIR`, so `lander attachment cat <id>` works immediately without a round-trip. Outputs take a larger 100 MiB cap than the 25 MiB message cap, since build outputs run bigger than what a human drags into a composer.

Attaching the same name twice yields two chips, not one: every blob is immutable, the store has no delete, and two publishes are two outputs. An earlier turn's chip therefore keeps serving exactly the bytes and the size that turn attached. (Outputs were once "artifacts" — a mutable name→blob slot per task, where a republish deleted what it displaced and left the earlier turn's ref dangling. A boot migration folded the surviving records into attachments.)
...
| `lander.land()` / `lander.wedge({ options })` / `lander.ask({ options })` / `await lander.ride({ date, time, await })` | act on the current task — bare `wedge()` just sets status, or pass `{ options: [{ id, label }] }` to raise a choice ask whose chosen label arrives as the next user message; `ask({ options })` is the same but advisory (no wedge, nothing in the list) |
| `await lander.ride({ clear: true })` | drop the current task's pending wakeup triggers; returns whether any were armed |
| `await lander.attachments.put(pathOrBytes, { name, mime })` / `.list()` | attach a file to the turn this task is riding (returns the ref), or list the files it has locally |
...
Each task row carries a **kebab (⋮) menu** with the status actions — **Wedge** / **Un-wedge** / **Land** / **Un-land** / **Launch** — plus **Archive**. Un-wedge and Un-land clear that status without waking the task. Only the items that would be both visible *and* enabled for the row's status appear (e.g. a landed task offers Wedge/Un-land/Archive but not Land; a riding task can't be archived), so the menu never shows a dead option. The menu is `position: fixed`, anchored to the kebab's live rect, so the scrolling list can't clip it, and arrow keys move between items. Archived tasks are hidden by default; the project dropdown has a **Show archived** toggle that merges them in (each row dimmed and tagged `archived`), where the kebab offers only **Restore**.
...
- `status` moves through `riding` (agent working) → `pacing` (idle) when a turn finishes; an agent can also set its own status — `wedged` (needs the user) or the terminal `landed` — via the `lander` CLI (see Self-management).
