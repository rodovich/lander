...
Where attachments are message **inputs**, artifacts are a task's **outputs**: named files a task publishes as it works. An artifact is a named slot on the task pointing at its latest version — publishing to the same name mints a fresh blob and repoints the slot. The displaced blob is **kept**: refs recorded on earlier messages point at it, and what a turn published is a record, not a mutable cell. The blobs reuse the same durable per-project store as attachments (`data/<project>/attachments/`), which has no delete at all, with a separate 100 MiB cap. Names are addressable (route segment + download filename) so they're validated strictly (`^[A-Za-z0-9_][A-Za-z0-9._-]{0,127}$` — no leading dot, no slash).

An agent publishes with `lander artifact put <path> [--name <n>] [--mime <m>]` (name defaults to the file's basename), lists its slots with `lander artifact ls`, and reads a slot's current bytes with `lander artifact cat <name>` — all scoped to the current task, which may publish only its own outputs. Each publish records a ref on the assistant message that generated it (the in-flight message when mid-turn, else the last assistant message), and the web UI renders those as **download rows at the bottom of that message**. Those rows still resolve by slot name, so a ref left by an earlier publish shows its own size against the latest bytes — a known wart being removed as artifacts fold into attachments.
